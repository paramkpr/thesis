...
class ExperimentRunner:
    ...
    def run_experiment(
        ...
        result.vanilla_balance_stats = self._analyze_balance(
            vanilla_hierarchy, color_ids
        )
        ...
        logger.info(
            f"[{name}] Experiment complete: cost ratio = {result.cost_ratio:.2f}"
        )

        return result

    def run_varying_size(
        self,
        base_name: str,
        dataset: str,
        sizes: List[int],
    ...
    def _calculate_cost(self, hierarchy: Hierarchy, points: np.ndarray) -> float:
        ...
        cost = n_leaves * (
            1 + imbalance_sum / len(internal_nodes) if internal_nodes else 0
        )
        return cost

    def _analyze_balance(
        self, hierarchy: Hierarchy, color_ids: List[List[int]]
    ) -> Dict[str, Any]:
        """
    ...
    def save_results_to_csv(self, filename: str = "experiment_results.csv"):
        ...
        for name, result in self.results.items():
            row = {
                ...
                "vanilla_mean_deviation": result.vanilla_balance_stats.get(
                    "mean_deviation", 0
                ),
                "fair_mean_deviation": result.fair_balance_stats.get(
                    "mean_deviation", 0
                ),
                "improvement_ratio": (
                ...
                if result.fair_balance_stats.get("mean_deviation", 0) > 0
                else 0,
            }
            data.append(row)
